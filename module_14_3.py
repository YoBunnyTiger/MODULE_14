...
async def get_buying_list(message: types.Message):
    products_info = [
        ("Красная Пилюля", "Помогает при простуде", 100, 'files/1.png'),
        ("Синяя Пилюля", "От сглазов и магов", 200, 'files/2.png'),
        ("Зеленая Пилюля", "+500 в Карму", 300, 'files/3.png'),
        ("Желтая Пилюля", "Храбрость и Отвага", 400, 'files/4.png')
    ]

    for product_name, description, price, img in products_info:
        await message.answer(f'Название: {product_name} | Описание: {description} | Цена: {price}₽')
        with open(img, "rb") as img_prod:
